...
def update_timing():
    ...
    try:
        ...
        status_label.config(text=f"⏱ Timings updated")
    except ValueError:
...
ttk.Button(buttons_frame, text="🎙 Start Fixed", command=start_fixed_recording).grid(row=0, column=1, padx=10)
ttk.Button(buttons_frame, text="⏸ Pause", command=pause_recording).grid(row=0, column=2, padx=10)
ttk.Button(buttons_frame, text="⏯ Resume", command=resume_recording).grid(row=0, column=3, padx=10)
ttk.Button(buttons_frame, text="⏹ Stop & Transcribe", command=stop_recording).grid(row=0, column=4, padx=10)
